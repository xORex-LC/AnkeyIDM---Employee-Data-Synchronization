...
| `extractor.py` | `Extractor` — входная граница source-потока: оборачивает `SourceRecord` в `TransformResult[None]`, мапит `SourceReadError`/`SourceParseError` в `SOURCE_READ_FAILED`/`SOURCE_PARSE_FAILED`, неизвестные исключения — в `SOURCE_ERROR` |
