...
def handler(ctx: BoundCommandContext, opts: Options, report_sink) -> CommandResult:
    ...
    include_normalized_items_value = (
        opts.include_normalized_items
        if opts.include_normalized_items is not None
        else True
    )
    ...
    try:
        ...
        return sqlite_cache_error_result(
            logger=ctx.logger, run_id=run_id, scope="normalize", exc=exc
        )
